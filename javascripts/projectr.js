...
(function() {
  ...
  Projectr = {
    ...
    _updateRepo: function() {
      ...
      cachedRepo = RepoCache.get(this._language());
      if (cachedRepo != null) {
        return this._updateIssueForRepo(cachedRepo);
      } else {
        return this._updateCachedRepos();
      }
    },
    ...
    _updateIssueForRepo: function(repo) {
      var _this = this;

      return $.ajax({
        url: this._issueURL(repo),
        dataType: 'jsonp'
      }).done(function(response) {
        var issue;

        if (response.meta['X-RateLimit-Remaining'] === '0') {
          return _this._limitExceeded();
        } else {
          issue = _.find(response.data, function(issue) {
            return issue.pull_request.html_url === null && issue.body !== '' && issue.body !== null;
          });
          if (issue) {
            return _this._displayIssue(issue);
          } else {
            return _this._updateRepo();
          }
        }
      });
    },
    _updateCachedRepos: function() {
      var _this = this;
      ...
      return $.ajax({
        ...
        if (response.meta['X-RateLimit-Remaining'] === '0') {
          ...
          RepoCache.set(_this._language(), response.data.repositories);
          return _this._updateRepo();
        }
      });
    },
    ...
    _displayIssue: function(issue) {
      $(this.issueSelector).html(_.template($('#issue_template').html(), {
        issue: issue
      }));
      return this._finishedLoading();
    },
    _issueURL: function(repo) {
      return "" + this.apiBaseUrl + "/repos/" + repo.owner + "/" + repo.name + "/issues";
    },
    _language: function() {
    ...
    _limitExceeded: function() {
      this._finishedLoading();
      $(this.issueSelector).addClass('rate-limit-exceeded').html('Request limit has been exceeded. Please try again from another IP Address');
  ...
  RepoCache = {
    get: function(language) {
      ...
      if (this._storage(language).length > 0) {
        repos = this._storage(language);
        repo = repos.pop();
        this.set(language, repos);
        return repo;
      }
    },
    ...
    _filter: function(repositories) {
      return repositories = _.filter(repositories, function(repo) {
        return repo.has_issues && repo.open_issues > 0;
      });
    },
  ...
  $(document).ready(function() {
    ...
    $('#language').change(function() {
      ...
      return issue = Projectr.updateIssue();
    });
    $('#new').click(function() {
      var issue;
    ...
    return $('#do_it').click(function() {
      return window.location = $('#issue a').attr('href');
    });
  });
