...
(function() {
  ...
  Projectr = {
    ...
    _updateRepo: function() {
      ...
      return $.ajax({
        ...
        if (response.meta['X-RateLimit-Remaining'] === '0') {
          ...
          repo = _.find(response.data.repositories, function(repo) {
            return repo.has_issues && repo.open_issues > 0;
          });
          return _this._updateIssueForRepo(repo);
        }
      });
    },
    _repoUrl: function() {
    ...
    _limitExceeded: function() {
      ...
      return $(this.issueSelector).html('Request limit has been exceeded. Please try again from another IP Address');
    }
  };
